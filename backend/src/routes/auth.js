...
import { createAuthErrorResponse, redeemCode, refreshAccessToken, logoutRefreshToken, exchangeGoogleAuthorizationCode } from '../services/authService.js';
...
export function createAuthRoute() {
  ...
  route.post('/google/exchange', async (c) => {
    try {
      const body = await parseJsonBody(c);
      if (!body) {
        return c.json({ error: 'Invalid JSON body' }, 400);
      ...
      const result = await exchangeGoogleAuthorizationCode(body.code, body.code_verifier, body.redirect_uri);
      return c.json(result);
    } catch (error) {
      const response = createAuthErrorResponse(error);
      return c.json(response.body, response.status);
