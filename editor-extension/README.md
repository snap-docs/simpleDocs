...
This optional Windows extension improves background capture in VS Code and Cursor.
...
1. Install the packaged `simpledocs-context-0.1.0.vsix` using **Extensions: Install from VSIX** in VS Code or Cursor.
...
4. The desktop app captures the selection and asks this extension for matching surrounding context.
...
The extension responds only when its window is focused, exactly one non-empty selection exists,
and that selection matches the desktop request (allowing CRLF/LF normalization).
...
the native terminal check and exact selection match reduce stale-editor matches. Identical text selected
in another pane can remain ambiguous. Custom keybindings or disabled accessibility can still prevent
the desktop app from obtaining an initial selection. Multiple selections fall back to native capture.
...
Package with `npx @vscode/vsce package --no-dependencies --out ../dist/simpledocs-context-0.1.0.vsix`.
